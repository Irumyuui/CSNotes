...
struct Fread {
	...
	template <class T> Fread& operator >> (T &val) {
		this->read(val);
		return *this;
	}
} fin;
...
inline void change(int l, int r, int oid, int &nid, int tag, int d) {
	nid = getNode();
	if (l == r) {
		seg[nid].val = d;
	} else {
...
inline int query(int l, int r, int id, int ql, int qr) {
	if (l == ql && r == qr) {
		return seg[id].val;
	} else {
		int mid = l + ((r - l) >> 1);
