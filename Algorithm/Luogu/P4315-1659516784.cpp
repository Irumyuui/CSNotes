...
void build(int l, int r, int id) {
	if (l == r) {
		...
		// #ifndef ONLINE_JUDGE
		// 	debug(l, rnk[l], ernk[l], w[ernk[l]], w[rnk[l]]);
		// #endif
...
void Cover(int a, int b, int val) {
	while (top[a] != top[b]) {
		...
		// #ifndef ONLINE_JUDGE
		// 	debug(a, b, top[a], top[b], fa[a], fa[b]);
		// #endif
	}
	if (dep[a] > dep[b]) {
		swap(a, b);
	...
	// #ifndef ONLINE_JUDGE
	// 	debug(a, b, dep[a], dep[b], dfn[a], dfn[b]);
	// #endif
...
void Solution() {
	...
	// #ifndef ONLINE_JUDGE
	// 	for (int i = 1; i <= n; ++ i) {
	// 		debug(i, son[i], dep[i], top[i], fa[i]);
	// 	}
	// #endif
	...
	while (1) {
		...
		if (opt == "Change") {
			...
			Cover(u, v, w);
				...
				for (int i = 1; i <= n; ++ i) {
					debug(i, query(1, n, 1, dfn[i], dfn[i]));
				}
