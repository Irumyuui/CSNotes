...
vector<Edge> edge[N];
void add_edge(int from, int to, int len, int id) {
	edge[from].push_back({to, len, id});
	edge[to].push_back({from, len, id});
}
...
int ernk[N], w[N];
int stamp = 0;
void dfs1(int from, int elen, int eid) {
	ernk[eid] = from;
	w[from] = elen;
	son[from] = -1;
	siz[from] = 1;
	for (auto [to, len, id] : edge[from]) {
		if (dep[to] == -1) {
			dep[to] = dep[from] + 1;
			fa[to] = from;
			dfs1(to, len, id);
